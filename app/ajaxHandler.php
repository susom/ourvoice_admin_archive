...
//            header("location:index.php?msg=$msg");
        ...
        case "transcribeAudio":
            $ajax_resp = true;
            $gs_ref     = !empty($_POST["gs_ref"]) ? filter_var($_POST["gs_ref"], FILTER_SANITIZE_STRING) : null;
            $proj_langs = !empty($_POST["proj_langs"]) ? filter_var_array($_POST["proj_langs"], FILTER_SANITIZE_STRING) : array();

            if ($gs_ref){
                $response = json_encode(transcribeSpeech($gs_ref, $proj_langs));
            }
            break;
    }
