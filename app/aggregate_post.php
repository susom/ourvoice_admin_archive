...
if(isset($_POST["deleteTag"])){
	...
	$result 	= $ds->push_data(cfg::$couch_url . "/" . cfg::$couch_proj_db . "/" . cfg::$couch_config_db, $payload);
	$_SESSION["DT"] = $payload;

	// GET ALL PHOTOS WITH TAG, REMOVE TAG FROM PHOTO, RESAVE
	$photos_w_tag 	= $ds->filterProjectByTags($pcode, array($deletetag));
	foreach($photos_w_tag["rows"] as $item){
		...
		$result 	= $ds->saveWalkData($_id, $payload);
	};
}
